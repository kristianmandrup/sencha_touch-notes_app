...
/**
 * This class allows the client to make use of message channels. A channel accepts messages which
 ...
 *
 * The {@link Ext.io.Device} class represents the device on which an app instance
 * is running. There is always a device object available for the current device.
 *
 ...
 *          });
 *
 * Instances of this class are also used to represent other devices running the same app,
 * and so devices can send messages to each other using the `send` method. The message
 * can be any plain old javascript object.
 *
 *          Ext.io.Device.find(
 *              {query: 'name:bob-smith-phone'},
 *              function(devices){
 *              
 *                  devices[0].send({
 *                      from: 'John', text: 'Hey!'
 ...
 *          );
 *
 * A device can listen for mesasges from other devices with the `receive` method.
 *
 *          Ext.io.Device.getCurrent({
 *             function(device){
 *
 ...
 * Ext.io.Channel.publish and Ext.io.Channel.subscribe.
 *
 * To send a message to all devices of a particular user the client would use
 * Ext.io.User.send and Ext.io.User.receive.
 *
 */
Ext.define('Ext.io.Device', {
    ...
    */
     ...
     *
     * @param {Function} callback The function to be called after receiving a message for this Device.
     * @param {String} callback.from The sending Device ID.
     * @param {Object} callback.message A simple Javascript object.
     *
     * @param {Object} scope The scope in which to execute the callbacks: The "this" object for
    ...
    receive: function(callback,scope) {
        ...
        if(!this.subscribedFn){
            this.subscribedFn = function(envelope) {
                console.log("subscribedFn.this",envelope, this.fireEvent);
 ...
 *
 * The {@link Ext.io.User} class represents a user. A current user object is 
 * available to the app once the user has registered or authenticated themselves.
 * This could happen on the web before download, or could occur in the app 
 * using the Ext.io.User.register and Ext.io.User.authenticate.
 *
 ...
 * object will always be abailable using Ext.io.User.getCurrent.
 *
 ...
 *      );
 *
 * Instances of this class are also used to represent other users running the same app,
 * and so users can send messages to each other using the `send` method. The message
 * can be any plain old javascript object.
 *
 *      Ext.io.User.find(
 *          {query: 'email:bob@isp.com'},
 *          function(users){
 *          
 *              user[0].send({
 *                  from: 'John', text: 'Hey!'
 ...
 *      });
 *
 * A client can listen for mesasges from other users with the `receive` method. .
 *
 *          Ext.io.User.getCurrent(
 *             function(user){
 *
 ...
 *          );
 *
 * If the user has multiple devices running the same app, and those apps have all made a 
 * call to Ext.io.User.receive, then the same message will be received by all those
 * app instances
 *
 * {@img user2.png}
 *
 ...
 * Ext.io.Channel.publish and Ext.io.Channel.subscribe.
 *
 * To send a message to a specific device the client would use
 * Ext.io.Device.send and Ext.io.Device.receive.
 *
 */
Ext.define('Ext.io.User', {
    ...
    */
    ...
    statics: {
        ...
        authenticate: function(options,callback,scope) {
            if(Ext.cf.util.ParamValidator.validateOptionsCallbackScope(null,arguments, "Ext.io.User", "authenticate")) {
                Ext.io.Group.getCurrent(function(group,err){
                    if(group){
                        group.authenticate(options,callback,scope);
                    }else{
                        callback.call(scope,group,err);
                    }
                });
         ...
         * @param {Object} options.callback.user The current {Ext.io.User} object if the call succeeded.
         * @param {Object} callback.err an error object.
         *
         * @param {Object} scope The scope in which to execute the callback. The "this" object for
         * the callback function.
         ...
         *
         * @param {Function} callback The function to be called after getting the User object.
         * @param {Object} options.callback.user The {Ext.io.User} object if the call succeeded.
         * @param {Object} callback.err an error object.
         *
         * @param {Object} scope The scope in which to execute the callback. The "this" object for
         * the callback function.
     ...
     * @param {Object} callback.devices The {Ext.io.Device[]} devices belonging to this User if the call succeeded.
     * @param {Object} callback.err an error object.
     *
     * @param {Object} scope The scope in which to execute the callbacks: The "this" object for
     * the callback function.
     ...
     * @private
     *
     * Called automticatlly by User.getCurrent so that user will get their messages when they are connected.
     *  Should not be called on any other user objects.
     *  Perhaps this should be an internal function or rolled into User.getCurrent.
     *
...
Ext.define('Ext.io.data.Proxy', {
    ...
    sync: function(store,callback,scope) {
        ...
        //JC notify watchers that there are changes in the store.
 ...
 *     })
 *
 * There are additional configuration options, which are documented in the Ext.Io.setup
 * method.
 *
 *
 * This class has has static methods to get the current {@link Ext.io.Device}
 * and {@link Ext.io.User} objects. Every app has a current Device object, and
 * only apps that have authenticated the user will have a current User object.
 *
 * A factory method is available for creating a {@link Ext.io.Channel} through which messages 
...
Ext.define('Ext.io.Io', {
     ...
     *
     * @param {Function} callback The function to be called after getting the channel.
     * @param {Ext.io.Channel} options.success.channel An {Ext.io.Channel} object.
     *
     * @param {Object} scope The scope in which to execute the callbacks: The "this" object for
     * the callback function.
     *
    ...
    getChannel: function(options,callback,scope) {
        if(Ext.cf.util.ParamValidator.validateOptionsCallbackScope([
                    { name: "name", type: 'string' },
            ],arguments, "Ext.io.Io", "getChannel")) {
                
            Ext.io.Io.init(function() {
                Ext.io.App.getCurrent(function(app,err) {
                    if(app){
                        app.getChannel(options,callback,scope);
                    }else{
                        callback.call(scope,app,err)
                    }
                });
...
Ext.define("Ext.io.ux.Authenticate", {
    ...
    requires: ["Ext.form.Panel", "Ext.form.FieldSet", "Ext.field.Password",  "Ext.field.Email"],
...
By default Ext.io.Controller will attempt to authenticate the user after it connects to the server. If the user already has a valid session then the "authenticated" event will fire with the user object.  If there is not a valid user session then Controller will create a view using the authenticationView as the type. It will then add the view to the viewport and present the user with a login screen.  If the user can enter a valid username and password then the "authenticated" event will fire and the user will be able to access their data. 

If the user chooses to register a new account then the "registered" will fire. 
...
If you don't want to automatically trigger the login panel when your application starts then set manualLogin to false:
        ...
        registerUser: function(reg) {
            if (reg.username.length > 0 && reg.password.length > 0 && reg.email.length > 0) {
                ...
                function(user, error) {
                        ...
                        this.loginPanel.showRegisterErrors(user);
                    }
                },
                this
                );
